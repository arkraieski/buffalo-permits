## Page rendering for the final static HTML.
##
## This file does not fetch or prepare data itself. It expects the build script
## (`scripts/build_site.R`) to source the other modules first, then call the
## functions below in this order:
##
## 1. `build_site_data()` from `R/data_prep.R`
...
## 5. `build_og_snapshot()` from this file
...
##
## Build a small render context once so page components can stay dumb.
...
format_generated_label <- function(generated_at) {
  format(
    lubridate::with_tz(generated_at, tzone = site_config$timezone),
    "%b %d, %Y %I:%M %p %Z"
  )
}

format_window_label <- function(window) {
  sprintf(
    "%s to %s",
...
build_crime_rank_table <- function(site_data) {
...
build_permit_rank_table <- function(site_data) {
      ...
      lapply(seq_len(nrow(data)), function(i) {
        htmltools::tags$tr(
          lapply(seq_along(data), function(j) {
            ...
            htmltools::tags$td(
              class = if (j %in% numeric_columns) "text-end" else NULL,
              cell_value
            )
          })
        )
...
## Keep site images simple until we deliberately add a real asset pipeline.
build_image_tag <- function(src, alt, class, eager = FALSE) {
  htmltools::tags$img(
    src = src,
    alt = alt,
    class = class,
...
build_page_head <- function() {
  ...
  og_image_url <- paste0(sub("/?$", "/", canonical_url), site_config$site$image)
...
## The page shell below is organized as reusable section builders rather than
## one long block of inline tags so the layout stays inspectable.
build_meta_item <- function(label, value) {
  htmltools::tags$div(
    class = "meta-item",
    htmltools::tags$div(class = "meta-term", label),
    htmltools::tags$div(class = "meta-detail", value)
  )
}

build_masthead <- function(render_context) {
  htmltools::tags$div(
    class = "masthead",
    htmltools::tags$section(
      ...
      htmltools::tags$div(
        class = "meta-row",
        build_meta_item("Window", render_context$window_label),
        build_meta_item("Updated", render_context$generated_label)
      )
    ),
    htmltools::tags$div(
...
build_chart_card <- function(title, chart_asset) {
  htmltools::tags$section(
    class = "chart-card",
    htmltools::tags$h3(title),
    htmltools::tags$img(
      src = chart_asset$src,
      alt = chart_asset$alt_text,
      class = "chart-image"
    )
  )
}

build_category_tables_card <- function(site_data) {
  crime_categories <- site_data$top_crime_types |>
  ...
  permit_categories <- site_data$top_permit_types |>
    dplyr::mutate(category = dplyr::coalesce(category, "Unknown")) |>
    dplyr::mutate(n = format_number_label(n)) |>
  ...
  htmltools::tags$section(
    ...
    htmltools::tags$div(
      ...
      htmltools::tags$div(
        ...
        build_data_table(crime_categories, numeric_columns = 2L)
      ),
      htmltools::tags$div(
        class = "mini-table",
        htmltools::tags$h4("Permit types"),
        build_data_table(permit_categories, numeric_columns = 2L)
      )
    )
  )
}

build_sources_card <- function() {
  htmltools::tags$section(
    class = "note-card",
...
build_photo_composition <- function() {
  htmltools::tags$div(
      ...
      htmltools::tags$main(
        ...
        htmltools::tags$div(
          ...
          htmltools::tags$section(
            ...
            ## Main interactive map is built in `R/maps.R`.
          ...
          htmltools::tags$section(
            ...
            htmltools::tags$div(
              ...
              ## Summary neighborhood choropleth is built in `R/maps.R`.
              make_summary_map(
                site_data$crime_neighborhoods,
          ...
          htmltools::tags$section(
            ...
            htmltools::tags$div(
              class = "chart-grid",
              build_chart_card("Total crime incidents per day", chart_assets$crime),
              build_chart_card("Total permits per day", chart_assets$permits)
            )
          ),
          htmltools::tags$section(
            class = "section-block",
            ...
            htmltools::tags$div(
              ...
              build_category_tables_card(site_data)
            )
          ),
          htmltools::tags$section(
